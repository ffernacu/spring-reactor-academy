...
public class SecurityContextRepository implements ServerSecurityContextRepository {
    ...
    public Mono<SecurityContext> load(ServerWebExchange exchange) {
        ...
        if(authHeader == null ||  !authHeader.startsWith("Bearer ") || authHeader.startsWith("bearer ")){
            return Mono.error(new ResponseStatusException(HttpStatus.UNAUTHORIZED, "No authorizado"));
        }

        final int TOKEN_POSITION = 1;
        String token = authHeader.split(" ")[TOKEN_POSITION];
        Authentication auth = new UsernamePasswordAuthenticationToken(token, token); //usuario y clave
